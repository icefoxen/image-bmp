...
impl<'a, W: Write + 'a> BMPEncoder<'a, W> {
    ...
    pub fn encode(
        ...
        // write image data
        match c {
            ColorType::RGB(8) => self.encode_rgb(image, width, height, row_pad_size, 3)?,
            ColorType::RGBA(8) => self.encode_rgba(image, width, height, row_pad_size, 4)?,
            ColorType::Gray(8) => self.encode_gray(image, width, height, row_pad_size, 1)?,
            ColorType::GrayA(8) => self.encode_gray(image, width, height, row_pad_size, 2)?,
            _ => {
...
fn get_unsupported_error_message(c: ColorType) -> String {
    format!(
...
fn get_pixel_info(c: ColorType) -> io::Result<(u32, u32, u32)> {
    let sizes = match c {
        ColorType::RGB(8) => (BITMAPINFOHEADER_SIZE, 3, 0),
        ColorType::RGBA(8) => (BITMAPV4HEADER_SIZE, 4, 0),
        ColorType::Gray(8) => (BITMAPINFOHEADER_SIZE, 1, 256),
        ColorType::GrayA(8) => (BITMAPINFOHEADER_SIZE, 1, 256),
        _ => {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                &get_unsupported_error_message(c)[..],
...
mod tests {
    ...
    fn round_trip_image(image: &[u8], width: u32, height: u32, c: ColorType) -> Vec<u8> {
        ...
        decoder.read_image_data().expect("failed to decode")
    }
