...
fn check_for_overflow(width: i32, length: i32, channels: usize) -> Result<(), ImageError> {
...
impl Bitfield {
    fn from_mask(mask: u32, max_len: u32) -> Result<Bitfield, ImageError> {
        if mask == 0 {
...
impl<R: Read + Seek> BMPDecoder<R> {
    ...
    fn read_bitmap_core_header(&mut self) -> Result<(), ImageError> {
    ...
    ///
    /// returns Err if any of the values are invalid.
    fn read_bitmap_info_header(&mut self) -> Result<(), ImageError> {
    ...
    fn read_bitmasks(&mut self) -> Result<(), ImageError> {
    ...
    #[cfg(feature = "ico")]
    #[doc(hidden)]
    pub fn read_metadata_in_ico_format(&mut self) -> Result<(), ImageError> {
    ...
    fn get_palette_size(&mut self) -> Result<usize, ImageError> {
    ...
    fn read_palette(&mut self) -> Result<(), ImageError> {
    ...
    fn read_palettized_pixel_data(&mut self) -> Result<Vec<u8>, ImageError> {
    ...
    fn read_32_bit_pixel_data(&mut self) -> Result<Vec<u8>, ImageError> {
        let mut pixel_data = self.create_pixel_data();
        let num_channels = self.num_channels();
    ...
    fn read_rle_data(&mut self, image_type: ImageType) -> Result<Vec<u8>, ImageError> {
    ...
    /// TODO: Fix this; either make it consume the reader, do what png::Decoder does and
    /// make it return a separate buffer, or something
